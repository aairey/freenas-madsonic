...
from madsonicUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (madsonic_key, madsonic_secret) = utils.get_madsonic_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=madsonic_key, secret=madsonic_secret)
    ...
    try:
        madsonic = models.Madsonic.objects.order_by('-id')[0]
        madsonic.enable = True
        madsonic.save()
    except IndexError:
        madsonic = models.Madsonic.objects.create(enable=True)

    try:
        form = forms.MadsonicForm(madsonic.__dict__, instance=madsonic, jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Madsonic data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.madsonic_control
...
def stop(request, plugin_id):
    ...
    try:
        madsonic = models.Madsonic.objects.order_by('-id')[0]
        madsonic.enable = False
        madsonic.save()
    except IndexError:
        madsonic = models.Madsonic.objects.create(enable=False)

    try:
        form = forms.MadsonicForm(madsonic.__dict__,
            instance=madsonic,
            jail_path=jail_path)
        form.is_valid()
        form.save()
    ...
    cmd = "%s onestop" % utils.madsonic_control
...
def edit(request, plugin_id):
    ...
    """
    Get the Madsonic object
    If it does not exist create a new entry
    """
    try:
        madsonic = models.Madsonic.objects.order_by('-id')[0]
    except IndexError:
        madsonic = models.Madsonic.objects.create()
    ...
    if request.method == "GET":
        form = forms.MadsonicForm(instance=madsonic, jail_path=jail_path)
    ...
    form = forms.MadsonicForm(request.POST, instance=madsonic, jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.madsonic_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True, close_fds=True)

        return JsonResponse(request, error=True, message="Madsonic settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('madsonic_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'madsonic', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen(["/usr/local/etc/rc.d/madsonic", "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.madsonic_icon, 'rb') as f:
        icon = f.read()
